function dxdt = integratingfunction_ABT199_no_feedback(t,x,Z,DE_params,P)
...
% treated apoptosis-ISR pathway for Molm-13 leukemia cells.
...
%--------------------------------------------------------------------------
%live cell number=x(25); dead cell number=x(26);
%--------------------------------------------------------------------------
...
%k_bim_tilde*(x(21)^n4/K_chop_bim^n4 + x(20)^n5/K_myc_bim^n5 + x(21)^n4*x(20)^n5/(K_chop_bim^n4*K_myc_bim^n5))...
%                     /(1.0+x(21)^n4/K_chop_bim^n4 + x(20)^n5/K_myc_bim^n5 + x(21)^n4*x(20)^n5/(K_chop_bim^n4*K_myc_bim^n5))...
%(3) Bim:
...
%(18) Bcl-2:aBak normalized to bcl2_0
...
f1_myc_A = (xlag1(23)+xlag1(24))^nf1_myc_A/(Kf1_myc_A_tilde^nf1_myc_A + (xlag1(23)+xlag1(24))^nf1_myc_A);

% g functions are for stability effects (1- increased stability; 
% 2- decreased stability). 
g1_myc_A = ABT^ng1_myc_A/(Kg1_myc_A_tilde^ng1_myc_A + ABT^ng1_myc_A);
...
% stability). 
f1_chop_A = ABT^nf1_chop_A/(Kf1_chop_A_tilde^nf1_chop_A + ABT^nf1_chop_A);

% g functions are for stability effects (1- increased stability; 
% 2- decreased stability). 
g1_chop_A = ABT^ng1_chop_A/(Kg1_chop_A_tilde^ng1_chop_A + ABT^ng1_chop_A);


%-------------------------------
%(20) c-Myc levels: 
dxdt(20) = kb_myc_tilde*(1.0 + kmax_myc_tilde*f1_myc_A)...  
        - (dp_myc_0_tilde...
        + (dp_myc_max_tilde - dp_myc_0_tilde)*g1_myc_A)*x(20);    

%(21) Chop levels:
dxdt(21) = kb_chop_tilde*(1.0 + kmax_chop_tilde*f1_chop_A)... 
         - (dp_chop_0_tilde...
         + (dp_chop_max_tilde - dp_chop_0_tilde)*g1_chop_A)*x(21);
               
%--------------------------------------------------------------------------
%(22) External ABT-199 concentration normalized to ABT_0:
dxdt(22) = -dE_ABT*x(22);

%(23) Free internal ABT-199 concentration normalized to ABT_0:
dxdt(23) = kA_uptake*x(22)...
         - kf_abt_bcl2_tilde*bcl2_0*x(2)*x(23) + kr_abt_bcl2_tilde*x(24)...
         - dA_tilde*x(23);   % Direct effects of drugs.   

%(24) ABT-199:bcl-2 complex concentration normalized to ABT_0:
...
%(25) Number of live cells:
dxdt(25) = (lambda_0_tilde + lambda_bcl2_tilde*x(2)^nv_1/(Bcl2_thresh^nv_1+x(2)^nv_1)...
         + lambda_myc_tilde*x(20)^nv_2/(Myc_thresh^nv_2+x(20)^nv_2)...
         - delta_casp3_tilde*aCasp3_frac^n12/(Casp3_thresh^n12+aCasp3_frac^n12))*x(25);

%(26) Dead cell count:
dxdt(26) = delta_casp3_tilde*aCasp3_frac^n12/(Casp3_thresh^n12+aCasp3_frac^n12)*x(25);
