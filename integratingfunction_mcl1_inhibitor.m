function dxdt = integratingfunction_mcl1_inhibitor(t,x,Z,DE_params,P)
% This integrating function is designed to mimic Bcl-2 inhibition in 
% combination with Mcl-1 inhibition.
% To accomplish this, we set Chop expression to only the regulatory effects 
% of ABT-199 treatment. We define a new variable, x(29), which is the old 
% Chop expression (for the ABT-199 and Tedizolid combination), and we use 
% this variable to regulate Mcl-1, since it will have the same degree of 
% Mcl-1 inhibition. 
...
%--------------------------------------------------------------------------
%old chop=x(29);  %Used to mimic Mcl-1 inhibition
...
%(1) Mcl-1:   %Note that we have replaced x(21) with x(29) here only:
dxdt(1) = k_mcl1_tilde*x(20)^n1/(K_myc_mcl1^n1+x(20)^n1)/(1.0+(x(29)/K_chop_mcl1)^n11)...
    + kb_mcl1_tilde... 
...
%(21) Chop levels:
% Set to ABT-199 regulation since we are not "administering Tedizolid" here:
...
%(29) Old chop levels with ABT-199 and Tedizolid combination treatment.
%Only using to estimate the degree of Mcl-1 inhibition:
dxdt(29) = kb_chop_tilde*(1.0...
         + kmax_chop_tilde*c_chop_f1/(1.0 + c_chop_f1))... 
         - (dp_chop_0_tilde...
         + (dp_chop_max_tilde - dp_chop_0_tilde)*c_chop_g1/(1.0 + c_chop_g1))*x(29);
