function dxdt = integratingfunction_Tedizolid_no_feedback(t,x,Z,DE_params,P)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
%external Tedizolid concentration/Ted_0=x(22); 
%intracellular Tedizolid concentration/Ted_0=x(23); 
%--------------------------------------------------------------------------
%live cell number=x(24); dead cell number=x(25);
%--------------------------------------------------------------------------
...
dxdt(1) = k_mcl1_tilde*x(20)^n1/(K_myc_mcl1^n1+x(20)^n1)/(1.0+(x(21)/K_chop_mcl1)^n11)...
    ...
    - dp_mcl1_tilde*x(1)...
    - kted_2_tilde*Ted_0*x(23)*x(1);  %drug interaction
...
dxdt(3) = k_bim_tilde*( (1.0 - x(20)^n5/(K_myc_bim^n5 + x(20)^n5))*x(21)^n4/(K_chop_bim^n4 + x(21)^n4)/(1.0 + (x(20)/K_myc_bim_2)^n13)...
    ...
    + kted_tilde*Ted_0*x(23)*x(11)*mcl1_0/bim_0;       % Tedizolid inactivates Mcl-1 which causes unbinding of Mcl-1:Bim complex.
...
dxdt(6) =  -dp_bax_tilde*x(6)... 
    ...
    + kted_tilde*Ted_0*x(23)*x(15)*mcl1_0/bax_0;        % Tedizolid inactivates Mcl-1 which causes unbinding of Mcl-1:aBax complex.
...
dxdt(7) =  -dp_bak_tilde*x(7)... 
    ...
    + kted_tilde*Ted_0*x(23)*x(16)*mcl1_0/bak_0;        % Tedizolid inactivates Mcl-1 which causes unbinding of Mcl-1:aBak complex.
...
dxdt(10) = -dp_mcl1_tilde*x(10) + kcc_tilde*casp3_0*x(1)*x(9)...
         + kted_2_tilde*Ted_0*x(23)*x(1)...
         + kted_tilde*Ted_0*x(23)*x(11)...
         + kted_tilde*Ted_0*x(23)*x(15)...
         + kted_tilde*Ted_0*x(23)*x(16);    
...
f1_myc_T = xlag2(23)^nf1_myc_T/(Kf1_myc_T_tilde^nf1_myc_T + xlag2(23)^nf1_myc_T);

% g functions are for stability effects (1- increased stability; 
% 2- decreased stability). 
g1_myc_T = x(23)^ng1_myc_T/(Kg1_myc_T_tilde^ng1_myc_T + x(23)^ng1_myc_T);
...
% stability). 
f1_chop_T = xlag4(23)^nf1_chop_T/(Kf1_chop_T_tilde^nf1_chop_T + xlag4(23)^nf1_chop_T);

% g functions are for stability effects (1- increased stability; 
% 2- decreased stability). 
g1_chop_T = xlag3(23)^ng1_chop_T/(Kg1_chop_T_tilde^ng1_chop_T + xlag3(23)^ng1_chop_T);

%(20) c-Myc levels: 
dxdt(20) = kb_myc_tilde*(1.0 + kmax_myc_tilde*f1_myc_T)...  
        - (dp_myc_0_tilde...
        + (dp_myc_max_tilde - dp_myc_0_tilde)*g1_myc_T)*x(20);  
    
%(21) Chop levels:
dxdt(21) = kb_chop_tilde*(1.0 + kmax_chop_tilde*f1_chop_T)... 
         - (dp_chop_0_tilde...
         + (dp_chop_max_tilde - dp_chop_0_tilde)*g1_chop_T)*x(21); 
     
%--------------------------------------------------------------------------     
%(22) External Tedizolid concentration:
dxdt(22) = -dE_Ted*x(22);

%(23) Intracellular Tedizolid concentration:
dxdt(23) = kT_uptake*x(22) - dT_tilde*x(23);

%--------------------------------------------------------------------------
% Active Caspase-3 fraction:
aCasp3_frac = x(9)/(x(8)+x(9));

%(24) Number of live cells:
dxdt(24) = ((lambda_0_tilde + lambda_bcl2_tilde*x(2)^nv_1/(Bcl2_thresh^nv_1+x(2)^nv_1)...
         + lambda_myc_tilde*x(20)^nv_2/(Myc_thresh^nv_2+x(20)^nv_2))/(1.0+(x(23)/q_Ted_tilde)^n_q_Ted)...
         - delta_casp3_tilde*aCasp3_frac^n12/(Casp3_thresh^n12+aCasp3_frac^n12))*x(24);

%(25) Dead cell count:
dxdt(25) = (delta_casp3_tilde*aCasp3_frac^n12/(Casp3_thresh^n12+aCasp3_frac^n12))*x(24);
